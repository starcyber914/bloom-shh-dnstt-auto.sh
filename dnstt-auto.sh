...
echo "==> BLOOM SHH DNSTT AUTO INSTALL STARTING..."

# Root check
if [ "$(id -u)" -ne 0 ]; then
  echo "[-] Run as root: sudo bash bloom-shh-dnstt-auto.sh"
  exit 1
fi

# Stop conflicting services
echo "==> Stopping old services..."
for svc in dnstt dnstt-server slowdns dnstt-smart dnstt-bloom-shh dnstt-bloom-shh-proxy; do
...
# DNSTT service
echo "==> Creating dnstt-bloom-shh.service..."
cat >/etc/systemd/system/dnstt-bloom-shh.service <<EOF
[Unit]
Description=BLOOM SHH DNSTT Server
...
# Proxy service
echo "==> Creating dnstt-bloom-shh-proxy.service..."
cat >/etc/systemd/system/dnstt-bloom-shh-proxy.service <<EOF
[Unit]
Description=BLOOM SHH DNSTT EDNS Proxy
After=network-online.target dnstt-bloom-shh.service
...
systemctl enable dnstt-bloom-shh.service
systemctl enable dnstt-bloom-shh-proxy.service
systemctl start dnstt-bloom-shh.service
systemctl start dnstt-bloom-shh-proxy.service

echo "======================================"
echo " BLOOM SHH DNSTT INSTALLED SUCCESSFULLY "
echo "======================================"
echo "DOMAIN  : ${TDOMAIN}"
echo "MTU     : ${MTU}"
echo "PUBLIC KEY:"
